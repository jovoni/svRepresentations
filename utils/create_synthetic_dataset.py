...
def create_tsv_dataset(df, dataset_path, ratio_train, use_original_seq):
    if df is None and dataset_path is not None:
        df = pd.read_csv(dataset_path, sep=";")
    
    if use_original_seq:
        sequences = df.ORIGINAL_SEQ.values
    else:
...
def create_homogenous_dataset(N, ratios, chromosome_name, same_sequence, put_original_sequence_first):
    """
    ...
    for i in range(N):
        if i == 0:
            if put_original_sequence_first:
                sv_type[i] = 0
                breakpoint_1[i] = 0
                breakpoint_2[i] = 0
                new_sequences[i] = str(original_sequences[i])
            else:
