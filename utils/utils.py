...
def exchange_base(current_base):
    bases = ["G", "T", "C", "A"]
    bases.remove(current_base.upper())

    # chose randomly
    return random.choice(bases)


def small_variants(seq, k):
    ...
    for i in range(len(seq)):
        if random.random() < k:
            ...
            seq = seq[:i] + exchange_base(seq[i]) + seq[i+1:]
    
    return seq, modified_indexes
...
def INV(seq):
    ...
    assert_sequence_validity(new_seq)
...
def DUP(seq):
    ...
    # Choose breakpoints
    l = len(seq)
    dup_length = random.randint(MIN_SV_LENGTH, int((MAX_LENGTH - l) / 2))
...
def prepare_data_for_model(path_folder, batch_size, index):
    # Read sequences
    if index is None:
        d = pd.read_csv(f"{path_folder}/data.tsv", sep="\t")
    else:
